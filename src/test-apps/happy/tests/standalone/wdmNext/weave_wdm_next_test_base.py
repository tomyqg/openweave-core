...
class weave_wdm_next_test_base(unittest.TestCase):
    def setUp(self):
        ...
        self.options = None
        self.test_client_iterations = self.__read_wdm_test_config(
            os.path.dirname(os.path.abspath(__file__))+"/wdm_test_config.json", "test_client_iterations")
    ...
    def __read_wdm_test_config(self, json_file, config_key):
        """
        read wdm-next test config
        """
        with open(json_file, 'r') as jfile:
            json_data = jfile.read()

        return json.loads(json_data)[config_key]
    ...
    def __run_wdm_test_between(self, nodeA, nodeB):
        ...
        options["test_client_iterations"] = self.test_client_iterations
